...
ShadowLayerForwarder::PaintedThebesBufferNet(ShadowableLayer* aThebes,
                                             ...
                                             const nsIntPoint& aBufferRotation,
                                             const SurfaceDescriptor& aNewFrontBuffer,
                                             unsigned int num, InfallibleTArray<PRUint32> sizes,
                                             InfallibleTArray<uint8> data, bool aDidSelfCopy)
{
  ...
  mTxn->AddPaint(OpPaintThebesBufferNet(NULL, Shadow(aThebes),
                                                      ...
                                                      aUpdatedRegion, 
                                                      num, 
                                                      (const InfallibleTArray<PRUint32>) sizes, 
                                                      (const InfallibleTArray<uint8> ) data, 
                                                      aDidSelfCopy));
}
                                          ...
                                          SurfaceDescriptor* aBuffer)
{
  ...
  if (PR_GetEnv("MOZ_LAYERS_FORCE_NETWORK_SURFACES")) {
    ...
    SurfaceDescriptorNetworked *a = new SurfaceDescriptorNetworked(aSize, id);
    *aBuffer = *a;
    MOZ_LAYERS_LOG(("> created new surface %x, id=%d", a, id));
    id++;
    return true;
  }
  ...
  *aBuffer = buffer->GetShmem();
  MOZ_LAYERS_LOG(("> created new shm surface %x", buffer.get()));
  return true;
}
